...
public class Fractal extends JComponent{
		...
		JPanel target;
		 Graphics2D g2;
	public void Fractal(int nFace1,int nVert1,double[][] vert1,int[][] face1,int denorm1, double lightX1,double lightY1,double lightZ1,int iter1,int dim1,int R1,int B1,int G1,boolean wireframe1){
	nFace=nFace1;nVert=nVert1;vert=vert1;face=face1;dim=dim1;iter=iter1;denorm=denorm1;lightX=lightX1;lightY=lightY1;lightZ=lightZ1;R=R1;B=B1;G=G1;wireframe=wireframe1;
    	...
    	screenVert=new int[nVert][2];
    	 ...
    	 Zsort = new int[nVert];
		  ...
		  g2=(Graphics2D)target.getGraphics();
		   bi=(BufferedImage)target.createImage(w,h);
